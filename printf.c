...
static char *__spf_ptr = 0;          //

static int vfprintf(int (*putc)(int), const char *format, va_list arg); //generic print
...
+=============================================================================+
*/
// __putc_spf() is the putc()function for sprintf()
static int __putc_spf(int ch){
	*__spf_ptr = (char)ch;	// just add the character to buffer
	 __spf_ptr++;
	 return ch;
}

static void long_itoa (long val, int radix, int len, int (*putc) (int)){
	char c, sgn = 0, pad = ' ';
	char s[20];
	int  i = 0;
...
__attribute__ ((used)) int printf(const char *format, ...) {
	...
	va_start(arg, format);
	vfprintf(putchar, format, arg);
	va_end(arg);

	return 0;
}

__attribute__ ((used)) int sprintf(char *buffer, const char *format, ...) {
	va_list arg;

	__spf_ptr = buffer;

	va_start(arg, format);
	vfprintf((&__putc_spf), format, arg);
	va_end(arg);

	*__spf_ptr ='\0';             // append end of string
